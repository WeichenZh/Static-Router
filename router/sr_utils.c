...
/*convert uint32 ip address to string xxx.xxx.xxx.xxx */
void int2ip_addr(uint32_t int_ip, char *ip_addr)
{
  int i=0;
  uint32_t curOctet;
  ...
  for (i=0;i<4;i++)
  {
    curOctet = (int_ip << 8*i) >> 24;
    sprintf(ip_addr+4*i, "%d", curOctet);
    ip_addr[4*i+3] = '.';
  }
  ip_addr[15] = '\0';
}
...
int check_checksum_ip(void *packet, unsigned int len)
{
  sr_ip_hdr_t *chk_pkg_hdr;
  uint16_t old_chksum;

  chk_pkg_hdr = (sr_ip_hdr_t *)packet;
  old_chksum = chk_pkg_hdr->ip_sum;
  chk_pkg_hdr->ip_sum = 0;

  if(old_chksum == cksum(packet, len))
    return 0;
  else
    return -1;
}
...
int check_checksum_icmp(void *packet, unsigned int len)
{
  sr_icmp_hdr_t *chk_pkg_hdr;
  uint16_t old_chksum;

  chk_pkg_hdr = (sr_icmp_hdr_t *)packet;
  old_chksum = chk_pkg_hdr->icmp_sum;
  chk_pkg_hdr->icmp_sum = 0;
